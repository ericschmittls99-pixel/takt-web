...
export const onRequestGet: PagesFunction<Env> = async ({ env }) => {
  const { results } = await env.DB.prepare(
    'SELECT id, title, due_date, done, favorite, employer_id, project_id, sort_order, created_at FROM todos ORDER BY done ASC, sort_order ASC, created_at ASC',
  ).all()
  return json(results)
}
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env }) => {
  ...
  const created = await env.DB.prepare(
    'INSERT INTO todos (title, due_date, done, employer_id, project_id, sort_order) VALUES (?, ?, ?, ?, ?, (SELECT COALESCE(MAX(sort_order), 0) + 1 FROM todos)) RETURNING *',
  )
    .bind(body.title.trim(), dueDate, done, employerId, projectId)
    .first()
